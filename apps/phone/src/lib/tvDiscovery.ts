...
export class TVDiscovery {
  ...
  start() {
    ...
    // 3. 주기적 재스캔
    this.scanInterval = setInterval(() => this.startHTTPDiscovery(), 3000);
  }
  ...
  private async discoverViaRelayServer() {
    if (this.callbacks.serverBase) {
      try {
        const res = await fetch(`${this.callbacks.serverBase}/announce`);
        if (res.ok) {
          const tvs: TVAnnouncement[] = await res.json();
          tvs.forEach(tv => this.handleAnnouncement(tv, this.callbacks.serverBase!));
          return;
        }
      } catch (e) {
        // 에러 시 폴백
  ...
  private async probeTV(ip: string): Promise<void> {
    ...
    const urls = [
      `http://${ip}:7180/announce`,
      `http://${ip}:7100/announce`,
