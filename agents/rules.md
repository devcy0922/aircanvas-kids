...
- 프레임 전송 금지: 폰→서버는 좌표 JSON/WebSocket 프레임만. 영상 프레임 절대 전송 금지(MVP).
- 좌표 규약: 모든 추적 좌표는 0..1 정규화. TV 렌더링 시점에 화면 해상도로 스케일.

## 4. 콘텐츠 규격 (v0.1)
- 테마 3종: 공룡 / 정글 / 바다. 테마당 10종 = 총 30종 아트워크.
...
- 검증 게이트: 어떤 기능 추가 후에도 `npm run build`(3앱) + `cargo check`(server) 통과를 원칙으로 한다.
